...
void oled_draw_char(uint8_t x, uint8_t y, char c)
{
  ...
  const byte draw_char_set_list[] = {
    I2C_OLED_ADDRESS,
    OLED_DATA_MODE,
    pgm_read_byte(&(FONT5x7[c - ' '][0])),
    pgm_read_byte(&(FONT5x7[c - ' '][1])),
    pgm_read_byte(&(FONT5x7[c - ' '][2])),
    pgm_read_byte(&(FONT5x7[c - ' '][3])),
    pgm_read_byte(&(FONT5x7[c - ' '][4])),
    0x00
  };

  send_package(draw_char_set_list, NELEMS(draw_char_set_list));
}
...
void oled_fill_black()
{
  const byte fill_black_set_list[] = {
    ...
    OLED_SET_MEMORY_ADDRESING_MODE__HORIZONTAL,
    OLED_SET_COLUMN_ADDRESS__, 0x00, 0x7F,
    OLED_SET_PAGE_ADDRESS__,   0x00, 0xB7
  };
